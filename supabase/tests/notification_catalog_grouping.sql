...
select is((select count(*) from private.notification_event_catalog),53::bigint,
  'source-controlled catalog contains every approved event family');
select is((select count(distinct category) from private.notification_event_catalog),36::bigint,
  'event families map to exactly 36 public categories');
