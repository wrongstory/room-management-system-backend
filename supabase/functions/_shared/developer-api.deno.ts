...
Deno.test("developer audit query accepts all 74 approved event types and rejects 75 before RPC", async () => {
  let calls = 0;
  const clients = {
    admin: {
      rpc: (_name: string, args: Record<string, unknown>) => {
        calls += 1;
        assert(
          (args.p_event_types as unknown[]).length === 74,
  ...
  assert(query.size === 74, "actual source enum inventory");
  ...
  assert(calls === 1, "all 74 accepted");
  ...
  try {
    ...
    throw new Error("75 must fail");
  } catch (error) {
    assert(
      error instanceof EdgeError && error.status === 400,
      "75 rejected with stable validation",
    );
  }
...
Deno.test("developer source migration head uses a stable migration name", () => {
  assert(
    expectedMigrationName === "assignment_unavailability_reassignment",
