...
Deno.test("developer audit query accepts all 51 approved event types and rejects 52 before RPC", async () => {
  let calls = 0;
  const clients = {
    admin: {
      rpc: (_name: string, args: Record<string, unknown>) => {
        calls += 1;
        assert(
          (args.p_event_types as unknown[]).length === 51,
  ...
  assert(query.size === 51, "actual source enum inventory");
  ...
  assert(calls === 1, "all 51 accepted");
  ...
  try {
    ...
    throw new Error("50 must fail");
  } catch (error) {
    assert(
      error instanceof EdgeError && error.status === 400,
      "50 rejected with stable validation",
    );
  }
...
Deno.test("developer source migration head uses a stable migration name", () => {
  assert(
    expectedMigrationName === "complaint_compensation_earning",
