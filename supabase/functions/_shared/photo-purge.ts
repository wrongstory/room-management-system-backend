...
export class PhotoPurgeError extends Error {
  constructor(readonly code: string, readonly statusCode = 503) {
    super(code);
    this.name = "PhotoPurgeError";
  }
}
export class PhotoPurgeProviderError extends PhotoPurgeError {
  constructor(readonly reason: PurgeReason) {
    super("PHOTO_PURGE_PROVIDER_UNAVAILABLE");
  }
}
...
export class PhotoPurgeWorker {
  ...
  async #files(
    ...
    for (const row of items) {
      ...
      const key = kind === "accepted"
        ? { p_object_id: uuid(row.objectId) }
        : { p_operation_id: uuid(row.operationId) };
      const args = {
        ...key,
        p_lease_version: version(row.leaseVersion),
  ...
  async #folders(
    ...
    for (const row of items) {
      ...
      const args = {
        p_folder_registry_id: uuid(row.folderRegistryId),
        p_lease_version: version(row.leaseVersion),
        p_claim_digest: claim,
      };
      const context = record(
...
export async function handlePhotoPurge(
  request: Request,
  secret: string | undefined,
  worker: () => Promise<PurgeRunResult>,
): Promise<Response> {
  ...
  try {
    ...
    if (
      !["/photo-purge", "/functions/v1/photo-purge"].includes(url.pathname) ||
      url.search
    ) return respond(404, { error: { code: "ROUTE_NOT_FOUND" } });
    if (request.method !== "POST") {
      return respond(405, { error: { code: "METHOD_NOT_ALLOWED" } });
    }
    if (!secret || secret.length < 32 || secret.length > 4096) {
      return respond(503, { error: { code: "PHOTO_PURGE_NOT_CONFIGURED" } });
    }
    const given = request.headers.get("x-photo-purge-secret") ?? "";
    if (given.length < 32 || given.length > 4096) {
      return respond(401, { error: { code: "INVALID_PHOTO_PURGE_SECRET" } });
    }
      ...
      try {
        for (let zeroChunks = 0; zeroChunks < 8; zeroChunks++) {
          ...
          if ((part.value?.byteLength ?? 0) > 0) {
            ...
            return respond(400, {
              error: { code: "INVALID_PHOTO_PURGE_REQUEST" },
            });
          }
    ...
    return respond(503, { error: { code: "PHOTO_PURGE_FAILED" } });
  }
}
