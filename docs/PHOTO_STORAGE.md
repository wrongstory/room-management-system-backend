...
2. 서버는 입력 5MiB, 12MP/5000px 기술상한을 검사하고 decode·방향 보정·metadata 제거·축소/재인코딩으로 JPEG/WebP **307,200바이트 이하**를 만든다.
   ...
   - Android Ultra HDR JPEG/R은 GContainer의 `Primary`·`GainMap`·선택 `MotionPhoto` 순서, 각 길이/padding, gain-map JPEG와 ISO-BMFF 경계를 모두 검증한 뒤 기본 SDR JPEG만 decode한다. `MotionPhoto=1`이지만 Samsung이 video item을 누락한 변형도 검증된 GainMap 직후의 남은 전체가 하나의 완전한 ISO-BMFF resource일 때만 허용한다. GainMap·영상·XMP는 저장하지 않는다.
