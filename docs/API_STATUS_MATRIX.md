...
- Issue #236 작업 브랜치의 source candidate는 **74 migrations / OpenAPI 0.5.0 126 paths / 136 operations**다. developer 객실 카탈로그·인원 관리와 예약 인원 상한을 포함하지만 아직 `dev`/`main` 병합, production DB/API 적용, hosted role smoke가 없어 production 수치는 위 73/120/130을 유지한다.
...
- [x] 74번째 append-only `developer_room_catalog_capacity` migration, private FORCE RLS preview 원장, service-role-only actor-bound RPC
