...
- **현재 사용**: 필요한 migration, secret, 계정/역할, runtime smoke까지 만족해 실제 클라이언트가 호출 가능한가.

### `Edge 배포`의 의미

Git에 TypeScript 코드가 있거나 DB RPC가 존재하는 것만으로는 Edge 배포가 아니다.

이 프로젝트에서 Edge 배포는 승인된 `main`의 Deno Function source를 운영 Supabase 프로젝트의 Edge runtime에 업로드해 실제 `/functions/v1/...` URL에서 실행되도록 만드는 것을 의미한다.
...
- 현재 `dev`: `2a73c1b76b89e54f6325bff22effae32bb790df1`
- #43 PR #48 최신 source: `058f45ca19fa0bf2dce58e5b6556c9658ad49966` — 아직 `dev` 미병합
- 운영 migration: **17건**
- 운영 Edge Functions:
  - `api` version 2 — ACTIVE
  - `reservation-scheduler` version 2 — ACTIVE
- production `api` 실제 HTTP operation: **13개**
- production `api`에는 아직 `/v1/availability/*`, `/v1/reservations/*`, 객실 상세/변경 API가 없음
- business admin: 아직 생성하지 않음
- scheduler actor/invoke secret: 아직 미설정, 현재 `reservation-scheduler` 503 fail-closed가 정상
- #48 developer API 6개는 source 완료 상태이나 production 미배포
...
PR #48 source는 독립 리뷰에서 P0/P1 0 판정을 받았지만 아직 `dev`에 병합되지 않았고 production migration/Edge 재배포도 하지 않았다.

| 체크 | Method / Path | 권한 | DB/RPC | Fastify HTTP | Edge source | Production Edge | 현재 사용 | 비고 |
|---|---|---|---|---|---|---|---|---|
| [ ] | `GET /v1/developer/overview` | developer only | 🟡 | — | 🟡 | ❌ | ❌ | PR #48 |
...
| 체크 | Method / Path | 권한 | DB/RPC | Fastify HTTP | Edge source | Production Edge | 현재 사용 | 비고 |
|---|---|---|---|---|---|---|---|---|
| [x] | `GET /v1/rooms` | admin | ✅ | ✅ | ✅ | ✅ | ⚠️ | endpoint는 배포됨. business admin 생성 전이라 실제 admin smoke 대기 |
...
| [ ] | scheduler heartbeat projection | developer 조회 | 🟡 | 🟡 | ❌ | ❌ | #48 source. migration + scheduler 재배포 필요 |
...
- `docs/DEVELOPER_OPERATIONS_API.md` — #48 병합 후
