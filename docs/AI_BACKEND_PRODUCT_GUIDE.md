...
- 현재 Phase C 작업 기준은 `dev@fa609099147dbf671a1db4b90bf4113760f5e1f3`의 61 migrations / OpenAPI 113 paths / 121 operations다. migration 순서는 57번째 `photo_slot_contract_v8` → 58번째 `extra_proof_photo_collection` → 59번째 `current_room_status_projection` → 60번째 `reservation_arrival_lifecycle_projection` → 61번째 `reservation_room_change_before_checkin`이며, Phase C `reservation_during_stay_room_move`는 기존 61개를 수정하지 않는 62번째 source candidate다. 공개 API 수는 113/121을 유지한다. 이 개발 snapshot은 아직 `main`/production 정본이 아니며 release 승인 전 운영 template·DB·Edge를 변경하지 않는다.
