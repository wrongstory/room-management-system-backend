...
- 이 문서 갱신의 개발 통합 기준: `dev@298a16294ad047bad11d7b84057acc1f61fe6423` — 64 migrations / OpenAPI 113 paths / 121 operations. #187 Phase A~C에 더해 #193 사진 retention과 #194 assignment PIN entitlement source 계약이 `dev`에 통합됐다. Issue #196 작업 브랜치의 65번째 migration / 114 paths / 122 operations는 이 통합 기준과 구분하는 미병합 후보다.
- 개발 migration 순서는 57번째 `photo_slot_contract_v8` → 58번째 `extra_proof_photo_collection` → 59번째 `current_room_status_projection` → 60번째 `reservation_arrival_lifecycle_projection` → 61번째 `reservation_room_change_before_checkin` → 62번째 `reservation_during_stay_room_move` → 63번째 `photo_retention_v2` → 64번째 `assignment_pin_entitlement`다. 기존 64개 migration은 변경하지 않고 #196 후보는 65번째 `reservation_bookability`만 append한다. 이 개발 snapshot과 작업 후보는 아직 `main`/production 정본이 아니며 release 승인 전 운영 template·DB·Edge를 변경하지 않는다.
...
객실 예약·준비 판단은 세 축을 합치지 않는다. `intervalBookable`은 요청한 미래 반개구간의 예약 가능성, `readinessStatus`/`checkInReady`는 현재 체크인·배정 준비, `pinSyncStatus`는 현재 PIN 동기화 상태다. 통합된 `dev`는 readiness/PIN 분리 projection까지만 제공하고, Issue #196 작업 브랜치는 일반 예약용 `intervalBookable` preview와 bounded from/to/cursor 범위 조회를 source 후보로 추가한다. 이 후보는 required gate·리뷰·`dev` 병합·release/main·production 배포가 남아 있으므로 운영 사용 가능 상태가 아니다. `allocation_ready`를 그 대체값으로 사용하지 않는다. 미래 기간은 예약 가능하면서 현재 체크인 준비는 불가할 수 있으며, preview 결과는 commit 성공 보장이 아니다.
...
- **#187 Phase C 확정 source/dev 계약 / production 미배포:** PR #190이 `dev@1571565b9e361e890cba6502aa3acbf9a08816c3`에 병합됐다. `reservations.room_id`는 최초 입실 계약 객실 이력으로 유지하고 private stay와 `[startsAt,endsAt)` room segment가 현재·미래 점유를 권위 있게 표현한다. `DURING_STAY`는 `serverNow <= effectiveAt < checkOutAt`, 실제 current source와 `VACANT + READY` target, segment/객실 version·fingerprint·TTL·idempotency를 모두 재검증한다. source segment 종료와 target segment 시작은 정확히 같은 시각이고 객실별·stay별 exclusion이 겹침을 막는다. 원 객실 checkout cleaning은 별도 obligation/target으로 exactly-once 만들며 마지막 예정 segment 객실이 최종 checkout obligation/target을 소유한다. 미래 이동의 source PIN lease는 즉시 폐기하지 않고 immutable cutoff를 기록해 effectiveAt 전 접근을 유지하고 이후 reveal/change/rotation과 Data API RLS에서 차단한다. safe `reservation.room_moved` audit과 비자기 admin notification/outbox는 같은 transaction이며 고객명·PIN·사진 URL·raw request/private ledger ID를 노출하지 않는다. 미래 이동 직후 `stay.currentRoomId`는 command 시각의 source이고 `sourceOutcome/targetOutcome`은 effectiveAt 결과다. generic room mutation은 계속 차단되며 이 62번째 migration과 API source는 release/main 승인과 운영 배포 전에는 production 사용 가능 상태가 아니다.
- **#196 현재 source 후보 / 미병합:** `POST /v1/reservations/bookability/preview`는 필수 `reservationType=standard`, strict RFC 3339 `[checkInAt,checkOutAt)`과 optional room type/exact eligible `excludeReservationId`를 받아 room candidate별 `intervalBookable`, 현재 `checkInReady`, 안정 reason, 한 `evaluatedAt`을 반환한다. `roomTypeIds` 생략과 `[]`는 모두 전체 유형이고 `excludeReservationId` 생략과 `null`은 모두 무제외다. PIN mismatch/unconfigured는 같은 snapshot에 실제 current check-in pending일 때만 readiness를 바꾸고 interval bookability를 바꾸지 않는다. bounded `GET /v1/reservations?from&to&roomId&cursor`는 최대 31일·50건, `(checkInAt,id)` 순서, actor/filter scoped opaque cursor와 한 DB `serverTime` snapshot을 사용하며 고객명·암호문·maid PII를 노출하지 않는다. query 없는 기존 목록 응답은 유지한다. preview는 read-only 힌트이고 최종 create/change transaction의 overlap·CAS 판정을 대체하지 않는다. `long_stay`/종료 미정/추가 room-move 정책은 이 후보 범위가 아니며 후속 계약에서 enum/nullable 의미를 확장한다. 65번째 append-only migration과 OpenAPI `0.4.0` 114 paths / 122 operations는 required gate·독립 리뷰·`dev` 병합 전 후보이며 production 완료로 표시하지 않는다.
...
- 배정 계획 소요시간은 네 객실 타입을 모두 포함한 versioned duration policy의 `confirmed` 값만 사용한다. fresh DB의 confirmed 0건은 정상이며 `ASSIGNMENT_PREVIEW_DURATION_POLICY_UNCONFIRMED`, `decisionReady=false`로 fail-closed한다. 55/65/70/80분은 계속 `[데모]`이고 template 값·평균·고정 시간으로 대체하지 않는다. 실제 청소 수행시간은 `cleaning_attempts.started_at → field_completed_at`, 객실 turnaround는 실제 checkout 시각 → `field_completed_at`으로 사후 계산한다. checkout 사진 템플릿의 `durationMinutes`는 선택적인 legacy/planning metadata일 뿐 배정·실행 완료 조건이 아니다.
...
- Issue #148/#152/#156/#165 후속으로 `main@6604b2215e06b9e9ebf0b3138e3716a000c57ddb`, production 56 migrations, `api` ACTIVE v16, OpenAPI `0.3.0` 109 paths / 117 operations와 5개 Edge bundle까지 반영됐다. GitHub Pages도 workflow run `35051144073`에서 같은 production OpenAPI를 다시 고정해 109/117 parity를 확인했다. 다만 annotated `v0.3.0` tag/GitHub Release, 예약 success mutation, Issue #112의 provider invoke secret·positive Web Push/heartbeat, Issue #137의 hosted Google target·서비스 계정·ACL·full resync/Cron smoke는 아직 완료 증거가 없다. source·bundle 배포와 운영 데이터/provider 활성화를 같은 완료 상태로 표시하지 않는다.
...
P2 배정부터 #112 Web Push provider, #73/#46/#128/#131/#136/#137/#140/#133/#156/#165까지 production source와 runtime에 반영됐다. production은 56 migrations / `api` ACTIVE v16 / OpenAPI 0.3.0 109 paths / 117 operations이며 네 checkout template도 슬롯 10/11/13/15, duration 미설정 상태의 v7로 게시됐다. 따라서 `CLEANING_TEMPLATE_NOT_CONFIGURED` 운영 차단의 구성 원인은 해소됐지만, 안전한 fixture가 없어 실제 예약 success mutation은 아직 검증하지 않았다.
