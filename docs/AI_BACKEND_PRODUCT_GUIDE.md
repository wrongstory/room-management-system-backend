...
- 현재 작업 후보 #179는 Decision A에 따라 57번째 append-only migration과 v8 사진 슬롯 계약을 추가한다. 아직 `dev`/`main`/production 정본이 아니며 실제 `extra-proof` 0~10장 collection 처리는 #180 완료 전 release하지 않는다.
