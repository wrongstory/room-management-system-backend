...
- production release 기준은 73 migrations / OpenAPI 120 paths / 130 operations이고, Issue #231과 #236까지 통합된 source 기준은 `dev@994d3b61e49f49671acf0c1192b8821ae7f77792` — 76 migrations / OpenAPI 0.5.0 126 paths / 136 operations이다. Issue #228 source 후보는 기존 76개를 수정하지 않는 77번째 migration과 OpenAPI 0.5.0 128 paths / 138 operations이며 아직 production 사용 가능 상태가 아니다.
...
- Issue #231과 #236은 `dev@994d3b61e49f49671acf0c1192b8821ae7f77792`에 통합되어 각각 75번째/76번째 append-only migration과 OpenAPI `0.5.0` 126 paths / 136 operations를 구성한다. 이는 아직 production 배포·운영 데이터 변경을 뜻하지 않는다.
...
관리자의 점유 상태 보정은 `primary_display_status`나 복합 status를 직접 저장하지 않는다. `POST /v1/rooms/{roomId}/occupancy-corrections`가 예약 ID, 목표 `occupied`, 과거/현재 `effectiveAt`, room CAS version, source-controlled reason과 Idempotency-Key를 받아 canonical segment를 retire/successor append하고 private correction 원장·safe occupancy event·audit·receipt를 같은 transaction에 남긴다. `occupied=true` 복원은 그 effectiveAt을 포함하는 **동일 객실의 기존 segment lineage**가 있을 때만 허용한다. A 객실을 vacant로 닫은 뒤 B 객실에서 occupied로 복원하는 요청은 정식 #187 room-move 원장을 우회하므로 `OCCUPANCY_CORRECTION_ROOM_MISMATCH`로 거부한다. segment 시작과 같은 vacant 보정은 zero-length successor 없이 전체 segment를 retire한다. active/password-complete business admin만 허용하고 maid/developer는 거부한다.

관리자가 카드의 `BLOCKED | OCCUPIED | ARRIVAL_PENDING | RESERVATION_PRESENT | CLEANING_REQUIRED | READY` 표시 분류를 객실별로 강제 조정할 때는 `POST /v1/rooms/{roomId}/display-status-overrides`를 사용한다. 이 명령은 source-controlled enum, room CAS, Idempotency-Key, reason, append-only override/audit를 남기며 `null`로 현재 override를 해제한다. 응답 projection은 원장에서 계산한 `canonical_primary_display_status`, 현재 `display_status_override`, 둘을 합친 `primary_display_status`를 함께 제공한다. override는 표시/운영 분류일 뿐 예약·stay·점유·청소·readiness·bookability를 만들거나 지우지 않는다. 특히 `BLOCKED` 표시 override만으로 실제 고객 배정이 차단되지 않으며, 실제 차단은 기존 operation-block/issue/candle/data-status command로 원인을 기록해야 한다.
...
- Issue #236은 `dev@994d3b61e49f49671acf0c1192b8821ae7f77792`의 76번째 `developer_room_catalog_capacity` migration으로 developer 전용 안전 카탈로그, 객실 유형 인원 preview/commit, 객실 추가, 객실 비활성화 preview/commit과 예약 인원 상한 재검증을 추가한다. 기존 인원 값과 기존 이력은 보존하며 OpenAPI 개발 정본은 `0.5.0` 126 paths / 136 operations다. 아직 `main` 병합, production migration/API 배포, hosted 역할 smoke는 완료되지 않았다.
