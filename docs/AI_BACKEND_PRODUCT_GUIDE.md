...
- #100 complaint/appeal/correction source는 `dev@e31559d922c49f059ed5880ac1de4f3f11d8ca74`에서 시작한 feature PR 검토 상태다. 이 source 후보는 37 migrations / 85 paths / 92 operations이며 원 승인 수익의 typed FK chain, 30일 접수, immutable decision/response/event와 current pointer CAS, 7일 1회 maid 응답, 종결·correction을 구현한다. #101 compensation, #102 adjustment, #103 외부 지급 결과와 main/recovery/production은 변경하지 않았고, 아직 `dev` 병합 또는 현재 사용 가능을 뜻하지 않는다.
