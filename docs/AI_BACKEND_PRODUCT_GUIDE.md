...
- **#187 Phase B fail-closed:** generic `PATCH /v1/reservations/{reservationId}`와 Data API/direct SQL의 실제 `room_id` 변경은 `RESERVATION_ROOM_CHANGE_DEDICATED_COMMAND_REQUIRED`로 막는다. 투숙 중 이동은 Phase C 전까지 `DURING_STAY_NOT_SUPPORTED`이고 stay/segment row를 만들지 않는다. 공개·배정·통보·attempt가 있는 청소 workflow는 `CLEANING_ASSIGNMENT_LOCKED`, active PIN lease는 `PIN_LEASE_ACTIVE`, target block은 `TARGET_ROOM_BLOCKED`, 겹침은 `TARGET_ROOM_OVERLAP`이다. 성공은 PII/PIN/fingerprint 없는 `reservation.room_moved` 감사만 남기고 승인된 비자기 수신자가 없으므로 notification/outbox를 만들지 않는다.
