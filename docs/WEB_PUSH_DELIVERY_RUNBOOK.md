...
- `VAPID_PUBLIC_KEYRING_JSON`은 API가 binding proof를 검증할 prior version→public key map이며 최대 5개다. delivery 시작 시 private pair ring과 version set·public key identity를 exact 대조하고 drift는 provider 호출 0, durable `PROVIDER_CONFIGURATION_ERROR` degraded heartbeat로 끝낸다.
...
3. API/Edge 배포 전에 모든 Function Secrets를 먼저 주입한다. 특히 public config endpoint도
   `VAPID_CURRENT_KEY_VERSION`, `VAPID_PUBLIC_KEY`, `VAPID_PUBLIC_KEYRING_JSON`이 없거나 malformed curve이면 fail-closed이므로 secrets-before-API 순서를 지킨다.
...
config 응답의 10분 binding proof는 auth user/profile/live session, key version, public-key identity, 발급·만료를
domain-separated HMAC으로 결합한다. register는 proof의 검증된 version만 immutable revision과 request hash에
사용한다. 회전 overlap 동안 prior public/private ring에 모두 남은 version의 proof는 재시도 가능하지만,
만료·변조·다른 actor/session·unknown/removed version은 거부한다. 응답 유실 재시도는 같은 proof와 같은
idempotency key를 보존해야 동일 logical result로 수렴한다.
...
heartbeat가 5분 이상 오래됐거나 Cron configured/active가 아니거나 조회 시점의 current VAPID config crypto validation이
실패하거나 overdue/dead-letter/operator-blocked가 하나라도
