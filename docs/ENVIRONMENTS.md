...
- 주급·알림함·검수 대기열의 opaque cursor는 각각 `PAYROLL_CURSOR_HMAC_SECRET`, `NOTIFICATION_CURSOR_HMAC_SECRET`, `INSPECTION_CURSOR_HMAC_SECRET`을 사용한다. 세 값은 UTF-8 32바이트 이상이어야 하며 서로 또는 다른 암호화 키·pepper·서비스 키와 재사용하지 않는다. Fastify와 Edge는 같은 목적의 값을 공유하되 로그, Issue, PR, 브라우저에 노출하지 않는다.
