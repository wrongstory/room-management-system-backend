...
- 주급·알림함·검수/객실 운영 조회의 opaque cursor는 각각 `PAYROLL_CURSOR_HMAC_SECRET`, `NOTIFICATION_CURSOR_HMAC_SECRET`, `INSPECTION_CURSOR_HMAC_SECRET`을 사용한다. 객실 운영 차단·이슈 cursor는 검수 cursor와 같은 운영 조회용 secret을 사용하되 payload scope에 actor·객실·stream·status·sort를 고정한다. 세 값은 UTF-8 32바이트 이상이어야 하며 서로 또는 다른 암호화 키·pepper·서비스 키와 재사용하지 않는다. Fastify와 Edge는 같은 목적의 값을 공유하되 로그, Issue, PR, 브라우저에 노출하지 않는다.
- 객실 PIN의 bounded AAD context는 별도 `ROOM_PIN_AAD_*` 변수가 아니라 실제 runtime mapping에서 만든다. Fastify는 `APP_ENV` + `SUPABASE_PROJECT_REF`, Edge는 `RUNTIME_ENVIRONMENT` + `SUPABASE_PROJECT_REF`를 새 쓰기에 사용한다. 복구 환경의 이전 revision 복호화는 DB에 보존된 exact context를 사용한다. key, AAD 전체 bytes, PIN/envelope를 로그·Issue·PR에 기록하지 않는다.
- Phase B/C Sheet worker는 `GOOGLE_SHEETS_SPREADSHEET_ID`, `GOOGLE_SHEETS_ROOM_PIN_TAB`, `GOOGLE_SHEETS_SERVICE_ACCOUNT_EMAIL`, `GOOGLE_SHEETS_SERVICE_ACCOUNT_PRIVATE_KEY`, `ROOM_PIN_SHEET_SYNC_INVOKE_SECRET`을 별도 secret/config로 받는다. 승인된 source mapping과 exact match하기 전 PIN key와 Google private key를 읽지 않는다. full-resync run은 네 target 요소의 canonical digest까지 immutable하게 고정한다. 현재 source에는 local/test synthetic target만 있으며 production/recovery mapping과 실제 Sheet ID/credential은 release 승인 전 추가하지 않는다.
