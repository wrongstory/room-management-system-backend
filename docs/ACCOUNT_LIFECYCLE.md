...
`POST /v1/auth/password`는 전용 `private.password_change_commands` receipt를 사용한다. 범위는 `(actor_profile_id, account.password.change, idempotency_key)`이고 한 actor의 미완료 변경은 하나뿐이다. 시작한 live session은 domain-separated SHA-256 digest로 결합해 같은 actor의 다른 세션이 key를 알아도 takeover할 수 없다. receipt에는 비밀번호 원문·Supabase 변환 비밀번호·hash/HMAC·재사용 가능한 verifier·token·raw session ID를 저장하지 않는다. 비밀이 아닌 server-generated operation marker만 receipt와 Auth `app_metadata`에 결합하며, `profile_id`/`role` metadata는 비밀번호 변경·초기화·역할 변경에서 함께 보존한다. 감사 원장에도 raw idempotency key나 marker 대신 actor/command/key의 SHA-256 scoped identity만 남긴다.

최초 실행은 현재 비밀번호를 Supabase Auth에 직접 검증한 뒤 receipt claim을 얻은 요청만 Auth를 한 번 변경한다. Auth 변경에는 receipt의 operation marker도 함께 기록한다. Auth 성공 뒤 DB 완료나 HTTP 응답이 유실되면 같은 key와 원 요청 body를 다시 보낸다. 서버는 **현재 Auth marker가 receipt marker와 일치하고 요청의 새 비밀번호도 현재 Auth 값과 일치할 때만** 같은 의도 효과를 증명해 `must_change_password=false`, 다른 session 폐기, 감사 1건, receipt 완료를 한 DB transaction으로 수렴시키고 204를 반환한다. 비밀번호에서 만든 fingerprint는 저장하지 않으므로 `request_hash`는 actor+command만 증명하며 `currentPassword` byte equality를 durable receipt로 비교하지 않는다. 클라이언트는 반드시 원 요청 body를 그대로 재전송해야 한다. 이후 다른 비밀번호 변경이나 관리자 초기화가 marker를 회전하므로, 과거 key에 현재 비밀번호를 넣어도 `IDEMPOTENCY_KEY_REUSED`다.

관리자 초기화도 완료 key의 marker가 현재 Auth marker와 같을 때만 외부 Auth 호출 없는 성공 replay다. 대상이 이후 개인 비밀번호를 변경해 marker가 달라졌다면 과거 초기화 key는 `IDEMPOTENCY_KEY_REUSED`로 거부하며 임시 비밀번호를 다시 설정하거나 더 최신 self-change receipt를 변경하지 않는다.
