...
`POST /v1/auth/password`는 전용 `private.password_change_commands` receipt를 사용한다. 범위는 `(actor_profile_id, account.password.change, idempotency_key)`이고 한 actor의 미완료 변경은 하나뿐이다. 시작한 live session은 domain-separated SHA-256 digest로 결합해 같은 actor의 다른 세션이 key를 알아도 takeover할 수 없다. receipt에는 비밀번호 원문·Supabase 변환 비밀번호·hash/HMAC·재사용 가능한 verifier·token·raw session ID를 저장하지 않는다. 비밀이 아닌 server-generated effect version만 receipt에 저장하고, `auth.users.encrypted_password`가 실제로 바뀔 때 최소 trigger가 `private.auth_password_versions`의 동일 version을 결합한다. shadow 원장에는 Auth hash나 파생값을 복사하지 않으며 raw table은 Data API와 service role에도 닫혀 있다. 감사 원장에도 raw idempotency key나 effect version 대신 actor/command/key의 SHA-256 scoped identity만 남긴다.

최초 실행은 현재 비밀번호를 Supabase Auth에 직접 검증한 뒤 receipt claim을 얻은 요청만 Auth를 한 번 변경한다. Auth 성공 뒤 DB 완료나 HTTP 응답이 유실되면 같은 key와 원 요청 body를 다시 보낸다. 서버는 **현재 private password version이 receipt version과 일치하고 요청의 새 비밀번호도 현재 Auth 값과 일치할 때만** 같은 의도 효과를 증명해 `must_change_password=false`, 다른 session 폐기, 감사 1건, receipt 완료를 한 DB transaction으로 수렴시키고 204를 반환한다. 비밀번호에서 만든 fingerprint는 저장하지 않으므로 `request_hash`는 actor+command만 증명하며 `currentPassword` byte equality를 durable receipt로 비교하지 않는다. 클라이언트는 반드시 원 요청 body를 그대로 재전송해야 한다. 이후 self-change·관리자 초기화·adapter 밖 GoTrue 비밀번호 변경은 모두 password version을 회전하므로, 과거 key에 현재 비밀번호를 넣어도 `IDEMPOTENCY_KEY_REUSED`다. 로그인처럼 password hash를 바꾸지 않는 Auth 갱신은 version을 회전하지 않는다.

관리자 초기화도 완료 key의 effect version이 현재 private password version과 같을 때만 외부 Auth 호출 없는 성공 replay다. Auth password update가 성공했지만 응답이 유실된 prepared 상태는 현재 version의 일치만으로 완료하지 않고, 등록 전화번호 뒤 4자리로 만든 의도한 임시 비밀번호를 실제 Auth에 검증하고 그 검증 session을 즉시 폐기한 뒤 version을 다시 확인해야 finalize한다. 대상이 이후 개인 비밀번호를 변경해 version이 달라졌다면 과거 초기화 key는 `IDEMPOTENCY_KEY_REUSED`로 거부하며 임시 비밀번호를 다시 설정하거나 더 최신 self-change receipt를 변경하지 않는다.

다른 key 또는 같은 key의 충돌 요청은 미완료 lease 동안 `PASSWORD_CHANGE_IN_PROGRESS`로 막는다. 다른 live session의 takeover는 `PASSWORD_CHANGE_SESSION_MISMATCH`다. lease 만료 복구에서 effect version과 새 비밀번호가 함께 일치하지 않으면 crash-before-Auth인지 다른 payload 재시도인지 password-derived evidence 없이 구분할 수 없으므로 receipt를 `inconsistent`로 고정하고 운영 확인을 요구한다. 관리자 reset은 target password-change advisory lock을 profile row보다 먼저 잡고, 기존 account reset receipt에 결합한 별도 effect version을 준비한다. 외부 Auth reset 성공과 private version 확인 뒤 finalize RPC가 `inconsistent` receipt를 `superseded`로 닫으며, Auth 실패 시에는 `reset_pending`을 유지해 성공으로 가장하지 않는다. 이후 새 임시 비밀번호의 self-change는 새 receipt로 시작한다.

모든 `currentPassword`/replay 비밀번호 확인은 Supabase Auth 호출 전에 `private.password_verification_rate_limits`를 소비한다. 이 원장은 actor당 정확히 한 행이므로 session·client·Idempotency-Key를 회전해도 10회/분 한도를 우회하거나 row cardinality를 늘릴 수 없다. 다른 actor는 서로 격리되고, 포화 행은 window가 끝날 때까지 더 이상 UPDATE하지 않는다. session과 client는 raw 값이 아닌 domain-separated digest evidence만 보관한다. 검증을 위해 생성된 Auth session은 즉시 local revoke하며 폐기에 실패하면 변경 준비를 계속하지 않는다.
