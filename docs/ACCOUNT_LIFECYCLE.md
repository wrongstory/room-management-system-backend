...
관리자 초기화도 완료 key의 effect version이 현재 private password version과 같을 때만 외부 Auth 호출 없는 성공 replay다. 대상이 이후 개인 비밀번호를 변경해 version이 달라졌다면 과거 초기화 key는 `IDEMPOTENCY_KEY_REUSED`로 거부하며 임시 비밀번호를 다시 설정하거나 더 최신 self-change receipt를 변경하지 않는다.
