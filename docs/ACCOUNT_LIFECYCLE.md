...
`POST /v1/auth/password`는 전용 `private.password_change_commands` receipt를 사용한다. 범위는 `(actor_profile_id, account.password.change, idempotency_key)`이고 한 actor의 미완료 변경은 하나뿐이다. 시작한 live session은 domain-separated SHA-256 digest로 결합해 같은 actor의 다른 세션이 key를 알아도 takeover할 수 없다. receipt에는 비밀번호 원문·Supabase 변환 비밀번호·hash/HMAC·재사용 가능한 verifier·token·raw session ID를 저장하지 않는다. 비밀이 아닌 server-generated operation marker와 Auth가 반환한 exact RFC 3339 `user.updated_at`만 완료 효과 증거로 보존하며, marker는 Auth `app_metadata`에 결합한다. `profile_id`/`role` metadata는 비밀번호 변경·초기화·역할 변경에서 함께 보존한다. 감사 원장에도 raw idempotency key나 marker/Auth version 대신 actor/command/key의 SHA-256 scoped identity만 남긴다.

최초 실행은 현재 비밀번호를 Supabase Auth에 직접 검증한 뒤 receipt claim을 얻은 요청만 Auth를 한 번 변경한다. Auth 변경에는 receipt의 operation marker도 함께 기록한다. Auth 성공 뒤 DB 완료나 HTTP 응답이 유실되면 같은 key와 원 요청 body를 다시 보낸다. 서버는 **현재 Auth marker와 exact `user.updated_at`이 receipt의 완료 증거와 일치하고 요청의 새 비밀번호도 현재 Auth 값과 일치할 때만** 같은 의도 효과를 증명해 `must_change_password=false`, 다른 session 폐기, 감사 1건, receipt 완료를 한 DB transaction으로 수렴시키고 204를 반환한다. 비밀번호에서 만든 fingerprint는 저장하지 않으므로 `request_hash`는 actor+command만 증명하며 `currentPassword` byte equality를 durable receipt로 비교하지 않는다. 클라이언트는 반드시 원 요청 body를 그대로 재전송해야 한다. 이후 다른 비밀번호 변경·관리자 초기화뿐 아니라 adapter 밖 Auth 비밀번호/metadata 갱신도 marker 또는 Auth version을 바꾸므로, 과거 key에 현재 비밀번호를 넣어도 `IDEMPOTENCY_KEY_REUSED`다.

관리자 초기화도 완료 key의 marker와 exact Auth version이 현재 Auth 상태와 모두 같을 때만 외부 Auth 호출 없는 성공 replay다. 대상이 이후 개인 비밀번호를 변경했거나 adapter 밖에서 Auth 상태가 바뀌어 marker 또는 version이 달라졌다면 과거 초기화 key는 `IDEMPOTENCY_KEY_REUSED`로 거부하며 임시 비밀번호를 다시 설정하거나 더 최신 self-change receipt를 변경하지 않는다.

다른 key 또는 같은 key의 충돌 요청은 미완료 lease 동안 `PASSWORD_CHANGE_IN_PROGRESS`로 막는다. 다른 live session의 takeover는 `PASSWORD_CHANGE_SESSION_MISMATCH`다. lease 만료 복구에서 marker와 새 비밀번호가 함께 일치하지 않으면 crash-before-Auth인지 다른 payload 재시도인지 password-derived evidence 없이 구분할 수 없으므로 receipt를 `inconsistent`로 고정하고 운영 확인을 요구한다. 관리자 reset은 target password-change advisory lock을 profile row보다 먼저 잡고, 기존 account reset receipt에 결합한 별도 Auth marker를 준비한다. 외부 Auth reset 성공과 marker 확인 뒤 finalize RPC가 `inconsistent` receipt를 `superseded`로 닫으며, Auth 실패 시에는 `reset_pending`을 유지해 성공으로 가장하지 않는다. 이후 새 임시 비밀번호의 self-change는 새 receipt로 시작한다.
