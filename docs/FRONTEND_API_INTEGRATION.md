...
Swagger UI 상단의 **OpenAPI JSON 내려받기**로 파일을 받을 수 있다. production URL은 #42 source가 release를 통해 `main`에 승격되고 Edge Function이 배포된 뒤 환경변수로만 주입한다. Supabase project ref나 운영 URL을 프론트 소스에 하드코딩하지 않는다.
