...
비밀번호 변경은 일반 account command와 분리된 private receipt state machine을 사용한다. actor 단위 advisory lock과 미완료 partial unique constraint로 동시에 한 요청만 Auth mutation claim을 얻고, 시작 session의 domain-separated SHA-256 digest로 다른 live session takeover를 차단한다. receipt는 `auth_pending | completed | failed | inconsistent` 상태, 안전한 command fingerprint, session digest, 무작위 claim digest, bounded lease만 저장하며 비밀번호/파생 verifier/token/raw session ID는 저장하지 않는다. Auth 성공 뒤 DB/HTTP 실패 복구는 caller가 다시 보낸 새 비밀번호를 Supabase Auth에 직접 검증하여 same-effect를 증명하고, DB completion transaction은 profile gate 해제·다른 session 폐기·audit exactly-once·receipt 완료를 함께 처리한다. lease 만료 시 새 비밀번호가 현재 Auth와 일치하지 않으면 crash-before-Auth와 payload 변경을 안전하게 구분할 evidence가 없으므로 rollback/실패 추정 대신 inconsistent로 고정한다.
