...
비밀번호 변경은 일반 account command와 분리된 private receipt state machine을 사용한다. actor 단위 advisory lock과 미완료 partial unique constraint로 동시에 한 요청만 Auth mutation claim을 얻고, 시작 session의 domain-separated SHA-256 digest로 다른 live session takeover를 차단한다. receipt는 `auth_pending | completed | failed | inconsistent | reset_pending | superseded` 상태, 안전한 command fingerprint, session digest, 무작위 claim digest, 비밀이 아닌 Auth operation marker, bounded lease만 저장하며 비밀번호/파생 verifier/token/raw session ID는 저장하지 않는다. Auth 성공 뒤 DB/HTTP 실패 복구는 현재 Auth marker와 caller가 다시 보낸 새 비밀번호를 함께 검증하여 same-effect를 증명하고, DB completion transaction은 profile gate 해제·다른 session 폐기·audit exactly-once·receipt 완료를 함께 처리한다. 후속 변경이나 관리자 reset은 marker를 회전시켜 과거 key를 무효화한다. lease 만료 시 marker/새 비밀번호가 함께 일치하지 않으면 crash-before-Auth와 payload 변경을 안전하게 구분할 evidence가 없으므로 rollback/실패 추정 대신 inconsistent로 고정한다. 관리자 reset은 global reset lock → target password advisory lock → row lock 순서로 직렬화하고, 외부 Auth 성공 뒤 별도 finalize RPC에서만 inconsistent receipt를 supersede한다. 완료된 reset key replay는 현재 Auth marker가 동일할 때만 no-op이며, marker가 바뀐 뒤에는 더 최신 self-change receipt를 건드리지 않고 stale conflict로 끝난다. Auth password 검증은 session/client를 row dimension으로 사용하지 않는 actor당 1행의 durable 10회/분 limiter를 먼저 거치며 포화 뒤 window 만료까지 write를 멈춘다.
