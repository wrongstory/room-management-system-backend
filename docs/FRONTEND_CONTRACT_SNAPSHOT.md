...
| `DOCS/19_ROOM_PIN_SHEET_CLEANING_HISTORY_DECISIONS.md` | 최우선 확정 | 사진 보존 기산점과 PIN entitlement 수명주기는 #193/#194의 63~64번째 migration으로 source/dev에 반영됐다. | 계약과 구현 회귀를 유지하되 production 배포와 hosted provider 활성화는 별도 gate로 추적한다. |
...
- #193의 63번째 `photo_retention_v2`가 위 구분을 source/dev에 반영해 **해결됨**이다. production worker 배포·주기 활성화는 별도다.
...
- #194의 64번째 `assignment_pin_entitlement`가 durable entitlement와 30초 reveal 분리를 source/dev에 반영해 **해결됨**이다. hosted provider·Google Sheets/Cron 활성화와 production 배포는 별도다.
