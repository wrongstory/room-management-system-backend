...
production `expectedMigration`과 DB head는 현재 56번째 `cleaning_template_duration_optional`로 일치합니다. 합산 개발 source의 #179는
57번째 `photo_slot_contract_v8`, #180은 58번째 `extra_proof_photo_collection`, #184는 59번째 `current_room_status_projection` 순서이므로, 이 source를 production에 배포하기 전에는 migration을 먼저 승인된
