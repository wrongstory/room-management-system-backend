...
production `expectedMigration`과 DB head는 현재 56번째 `cleaning_template_duration_optional`로 일치합니다. 개발 정본의 #179는
57번째 `photo_slot_contract_v8`, #184는 58번째 `current_room_status_projection`, #187 Phase A source 후보는 59번째 `reservation_arrival_lifecycle_projection`을 기대하므로, 이 source를 production에 배포하기 전에는 migration을 먼저 승인된
