...
현재 production에는 Edge API와 OpenAPI JSON이 배포돼 있다. Supabase 기본 도메인은 HTML 응답을 `text/plain`으로 바꾸므로 production `/docs` route 자체는 존재하지만 인터넷 Swagger UI로 렌더링되지 않는다. 사람용 운영 문서는 GitHub Pages 읽기 전용 Swagger 포털에서 제공한다.
