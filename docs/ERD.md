...
- #137 `room_pin_sheet_full_resync_runs/items`는 operator/session/CAS/idempotency에 묶인 immutable snapshot과 실행 상태를 private FORCE RLS로 보존한다. #230 이후 121은 초기 seed 설명일 뿐 영구 제약이 아니며 요청 시점의 `active` 객실을 정확히 1~500건, sheet row 2..501 범위에 고정한다. run은 source-controlled exact target digest에 고정되고 incremental outbox와 같은 singleton fence를 쓴다. 일반 run은 자기 자신을 가리키는 immutable `recovery_root_run_id`를 만들며 recovery는 exact-fence blocked predecessor의 기존 root만 상속한다. root는 실행 CAS가 아니고 claim/lease fence 검증은 별도로 유지된다. 성공 시 snapshot room이면서 provider marker 이전 생성된 outbox와, 요청보다 과거인 같은-root blocked run 최대 32건만 supersede한다. 상한 초과/부분 정리는 marker 보존 `DB_SETTLE_UNCERTAIN`으로 닫고 marker 이후 새 PIN 변경과 unrelated/newer lineage는 남긴다. 공개 status와 developer audit에는 bounded count/time/stable code만 있고 PIN/envelope/provider identity는 없다.
- `ROOMS.catalog_status`는 `active → retired` 단방향이다. developer create/retire만 lifecycle을 바꾸고, 예약·stay segment·비종결 청소·미해결 이슈/차단/PIN workflow가 남은 객실은 은퇴할 수 없다. retired row와 모든 FK 이력은 유지하되 현재 운영·bookability·배정·Sheet projection에서는 제외한다.
