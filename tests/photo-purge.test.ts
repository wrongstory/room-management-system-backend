...
import { handlePhotoPurge, PhotoPurgeProviderError, PhotoPurgeWorker, type PurgeProvider, type PurgeRpc } from '../src/modules/photos/photo-purge.js';
...
function setup(options: { remove?: () => Promise<'deleted' | 'not_found'>; exists?: () => Promise<boolean>; contextError?: boolean; settleError?: boolean; budget?: boolean } = {}) {
  ...
  const db: PurgeRpc = { rpc: async (name, value) => {
    ...
    if (name === 'claim_due_photo_purges') return { data: { items: [{ objectId, leaseVersion: 1, purgeAfter: '2026-09-09T00:00:00.000001Z' }] }, error: null };
    if (name === 'claim_due_photo_orphan_purges' || name === 'claim_due_photo_folder_purges') return { data: { items: [] }, error: null };
    ...
    return { data: { status: 'purged', providerFileId: 'never reflect raw' }, error: options.settleError ? new Error('secret') : null };
  } };
  ...
  let clocks = 0;
  return { calls, args, worker: new PhotoPurgeWorker(db, provider, () => options.budget && clocks++ ? 45000 : 0) };
}
describe('photo retention worker (no real Google calls)', () => {
  it('claims at most ten items across accepted, orphan and folder phases', async () => {
    ...
    const db: PurgeRpc = { rpc: async (name, args) => {
      ...
      if (name === 'claim_due_photo_purges') return { data: { items: ids.slice(0, 8).map(objectId => ({ objectId, leaseVersion: 1 })) }, error: null };
      if (name === 'claim_due_photo_orphan_purges') return { data: { items: ids.slice(8).map((objectId, i) => ({ objectId, operationId: ids[i], leaseVersion: 1 })) }, error: null };
  ...
  it('verifies an empty folder before deleting and settles non-empty as a safe retry', async () => {
    ...
    const db: PurgeRpc = { rpc: async (name, args) => {
      if (name === 'claim_due_photo_purges' || name === 'claim_due_photo_orphan_purges') return { data: { items: [] }, error: null };
      if (name === 'claim_due_photo_folder_purges') return { data: { items: [{ folderRegistryId: folderId, leaseVersion: 1 }] }, error: null };
  ...
  it('DB µs due authorization precedes delete and fenced callback; no caller clocks', async () => {
    const s = setup(); expect(await s.worker.run()).toEqual({ claimed: 1, deleted: 1, notFound: 0, retryable: 0, deferred: 0, acceptedClaimed: 1, orphanClaimed: 0, folderClaimed: 0 });
...
describe('server-only exact purge route', () => {
  ...
  const run = async () => ({ claimed: 0, deleted: 0, notFound: 0, retryable: 0, deferred: 0, acceptedClaimed: 0, orphanClaimed: 0, folderClaimed: 0, providerId: 'must_not_reflect' });
