...
describe('Fastify photo parity through actual raw parser/router', () => {
  it('raw smartphone JPEG over 300KiB succeeds, over 5MiB rejects; malformed MIME and aliases do not execute', async () => {
    ...
    try {
      const good=await app.inject({method:'POST',url,headers:{'content-type':'image/jpeg','idempotency-key':'synthetic-key-01'},payload:Buffer.from(pad(307201))});
      expect(good.statusCode).toBe(200);expect(good.json().status).toBe('accepted');expect(good.headers['cache-control']).toBe('no-store');
      const large=await app.inject({method:'POST',url,headers:{'content-type':'image/jpeg','idempotency-key':'synthetic-key-02'},payload:Buffer.from(pad(PHOTO_INPUT_MAX_BYTES + 1))});
