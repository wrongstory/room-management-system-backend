...
function base64ToBytes(value: string, expectedLength?: number): Uint8Array {
  try {
    if (!value || !/^[A-Za-z0-9+/]+={0,2}$/.test(value)) fail('ROOM_PIN_ENVELOPE_INVALID');
    const binary = atob(value);
    const result = Uint8Array.from(binary, (character) => character.charCodeAt(0));
    if (bytesToBase64(result) !== value || (expectedLength !== undefined && result.length !== expectedLength)) {
...
function validateBinding(config: RoomPinCryptoConfig, roomId: string, pinVersion: number): void {
  if (!versionPattern.test(config.keyVersion) || !bindingPattern.test(config.environment)
  ...
  if (entries.length > 5 || Object.hasOwn(config.keyring, config.keyVersion)) fail('ROOM_PIN_CRYPTO_CONFIG_INVALID');
  const keyIdentities = new Set<string>([config.key]);
  base64ToBytes(config.key, 32);
  for (const [version, key] of entries) {
    if (!versionPattern.test(version) || keyIdentities.has(key)) fail('ROOM_PIN_CRYPTO_CONFIG_INVALID');
...
export function canonicalRoomPin(roomNumber: string, pinDigits: string): string {
  if (typeof roomNumber !== 'string' || !/^[A-Za-z0-9]{1,32}$/.test(roomNumber)
...
export async function encryptRoomPin(
  ...
  if (!/^[A-Za-z0-9]{1,32}-[0-9]{4,8}$/.test(canonicalCredential)) fail('INVALID_ROOM_PIN');
  const nonce = config.nonce === undefined ? crypto.getRandomValues(new Uint8Array(12)) : new Uint8Array(config.nonce);
  if (nonce.length !== 12) fail('ROOM_PIN_ENVELOPE_INVALID');
  try {
    const key = await crypto.subtle.importKey('raw', bufferSource(base64ToBytes(config.key, 32)), 'AES-GCM', false, ['encrypt']);
    const combined = new Uint8Array(await crypto.subtle.encrypt(
      { name: 'AES-GCM', iv: bufferSource(nonce), additionalData: bufferSource(aad(config.environment, config.projectRef, roomId, pinVersion)), tagLength: 128 },
      key,
...
export async function decryptRoomPin(
  ...
  if (envelope.envelopeFormat !== 1 || !versionPattern.test(envelope.keyVersion)) fail('ROOM_PIN_ENVELOPE_INVALID');
  try {
    ...
    const key = await crypto.subtle.importKey('raw', bufferSource(base64ToBytes(encodedKey, 32)), 'AES-GCM', false, ['decrypt']);
    const plaintext = new TextDecoder('utf-8', { fatal: true }).decode(await crypto.subtle.decrypt(
      { name: 'AES-GCM', iv: bufferSource(base64ToBytes(envelope.nonceBase64, 12)),
        additionalData: bufferSource(aad(envelope.aadEnvironment, envelope.aadProjectRef, roomId, pinVersion)), tagLength: 128 },
      key,
      bufferSource(combined)
    ));
    if (!/^[A-Za-z0-9]{1,32}-[0-9]{4,8}$/.test(plaintext) || !plaintext.startsWith(`${roomNumber}-`)) {
      fail('ROOM_PIN_DECRYPT_FAILED');
    }
    return plaintext;
  } catch (error) {
    if (error instanceof RoomPinCryptoError && error.code === 'ROOM_PIN_KEY_UNAVAILABLE') throw error;
    fail('ROOM_PIN_DECRYPT_FAILED');
  }
}
