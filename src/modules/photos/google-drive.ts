...
export class GoogleDriveProvider implements PhotoProvider {
  ...
  async #purgeRequest(path: string, init: RequestInit = {}): Promise<Response> {
  ...
  async purgeRemove(fileId: string): Promise<'deleted' | 'not_found'> {
    const response = await this.#purgeRequest(`files/${id(fileId)}`, { method: 'DELETE' });
    if (response.status === 404) return 'not_found';
    if (response.status !== 204) throw new PhotoPurgeProviderError('PROVIDER_ERROR');
    return 'deleted';
  }
  async purgeExists(fileId: string): Promise<boolean> {
    const response = await this.#purgeRequest(`files/${id(fileId)}?fields=id`);
  ...
  async purgeEmptyFolder(folderId: string, parentId: string): Promise<'empty' | 'not_found' | 'not_empty'> {
    const response = await this.#purgeRequest(`files/${id(folderId)}?fields=id,mimeType,parents,trashed,shared`);
    if (response.status === 404) return 'not_found';
    const row = await this.#json(response);
    ...
    const children = await this.#purgeRequest(`files?${query}`);
