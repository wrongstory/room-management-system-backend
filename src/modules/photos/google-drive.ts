...
export class GoogleDriveProvider implements PhotoProvider {
  ...
  #signal(deadlineAt?: number): AbortSignal {
    const remaining = deadlineAt === undefined ? 10000 : Math.floor(deadlineAt - Date.now());
    ...
    this.#refresh ??= (async () => {
      try {
        const response = await this.#fetch('https://oauth2.googleapis.com/token', {
          method: 'POST', redirect: 'error', signal: this.#signal(deadlineAt),
  ...
  async #request(path: string, init: RequestInit = {}, upload = false, deadlineAt?: number): Promise<Response> {
    ...
    try {
      const response = await this.#fetch(`https://www.googleapis.com/${upload ? 'upload/' : ''}drive/v3/${path}`, {
        ...init, redirect: 'error', signal: this.#signal(deadlineAt),
  ...
  async generateUploadIds(): Promise<[string, string, string]> {
    ...
    const response = await this.#request('files/generateIds?count=3&space=drive&type=files');
    if (!response.ok) return unavailable();
    const ids = (await this.#json(response)).ids;
    if (!Array.isArray(ids) || ids.length !== 3) return unavailable();
    const result: [string, string, string] = [id(ids[0]), id(ids[1]), id(ids[2])];
  ...
  async ensureFolder(folder: DriveFolder): Promise<void> {
    ...
    const [parentMetadata, existing] = await Promise.all([
      ...
      (async () => {
        const response = await this.#request(`files/${folderId}?fields=${fields}`);
  ...
  async #purgeRequest(path: string, deadlineAt: number, init: RequestInit = {}): Promise<Response> {
    let response: Response;
    try { response = await this.#request(path, init, false, deadlineAt); }
    catch { throw new PhotoPurgeProviderError('NETWORK_ERROR'); }
    if (response.status === 429) throw new PhotoPurgeProviderError('RATE_LIMITED');
    if (!response.ok && response.status !== 404) throw new PhotoPurgeProviderError('PROVIDER_ERROR');
    return response;
  }
  /** Only a DB-fenced due/retired identity can reach this internal method. */
  async purgeRemove(fileId: string, deadlineAt = Date.now() + 10000): Promise<'deleted' | 'not_found'> {
    const response = await this.#purgeRequest(`files/${id(fileId)}`, deadlineAt, { method: 'DELETE' });
    if (response.status === 404) return 'not_found';
    if (response.status !== 204) throw new PhotoPurgeProviderError('PROVIDER_ERROR');
    return 'deleted';
  }
  async purgeExists(fileId: string, deadlineAt = Date.now() + 10000): Promise<boolean> {
    const response = await this.#purgeRequest(`files/${id(fileId)}?fields=id`, deadlineAt);
    if (response.status === 404) return false;
    if ((await this.#json(response)).id !== fileId) throw new PhotoPurgeProviderError('PROVIDER_ERROR');
    return true;
  }
  /** Not deletion authority: caller must already own the DB retirement/admission barrier. */
  async purgeEmptyFolder(folderId: string, parentId: string, deadlineAt = Date.now() + 10000): Promise<'empty' | 'not_found' | 'not_empty'> {
    const response = await this.#purgeRequest(`files/${id(folderId)}?fields=id,mimeType,parents,trashed,shared`, deadlineAt);
    if (response.status === 404) return 'not_found';
    const row = await this.#json(response);
    if (row.id !== folderId || row.mimeType !== 'application/vnd.google-apps.folder' || row.shared !== false || row.trashed !== false || !Array.isArray(row.parents) || row.parents.length !== 1 || row.parents[0] !== id(parentId)) throw new PhotoPurgeProviderError('PROVIDER_ERROR');
    // Parent-only existence check, including trashed children: no filename search and no pagination scan.
    const query = new URLSearchParams({ q: `'${id(folderId)}' in parents`, pageSize: '1', fields: 'files(id),nextPageToken,incompleteSearch' });
    ...
    if (!Array.isArray(data.files) || data.incompleteSearch === true || data.nextPageToken) return 'not_empty';
