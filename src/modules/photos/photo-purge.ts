...
export interface PurgeProvider {
  ...
  purgeEmptyFolder(folderId: string, parentId: string): Promise<'empty' | 'not_found' | 'not_empty'>;
}
export interface PurgeRpc { rpc(name: string, args: Record<string, unknown>): PromiseLike<{ data: unknown; error: unknown }> }
export interface PurgeRunResult {
  claimed: number; deleted: number; notFound: number; retryable: number; deferred: number;
...
export class PhotoPurgeWorker {
  ...
  async rpc(name: string, args: Record<string, unknown>): Promise<unknown> {
    ...
    try {
      const timeout = new Promise<never>((_, reject) => { timer = setTimeout(() => reject(new PhotoPurgeError('PHOTO_PURGE_FAILED')), 5000); });
  ...
  async #claim(name: string, claim: string, limit: number): Promise<Record<string, unknown>[]> {
    if (limit === 0) return [];
    const batch = record(await this.rpc(name, { p_claim_digest: claim, p_limit: limit }));
    if (!Array.isArray(batch.items) || batch.items.length > limit) return failed();
    return batch.items.map(record);
  }
  #hasBudget(startedAt: number): boolean { return this.clock() - startedAt < PHOTO_PURGE_RUN_BUDGET_MS - 35000; }
  async #remove(fileId: string): Promise<{ outcome: 'deleted' | 'not_found' | 'retryable'; reason: PurgeReason | null }> {
    try { return { outcome: await this.provider.purgeRemove(fileId), reason: null }; }
    catch (error) {
      let reason: PurgeReason = error instanceof PhotoPurgeProviderError ? error.reason : 'NETWORK_ERROR';
      try { if (!await this.provider.purgeExists(fileId)) return { outcome: 'not_found', reason: null }; }
      catch { reason = 'NETWORK_ERROR'; }
      return { outcome: 'retryable', reason };
    }
  }
  async #files(items: Record<string, unknown>[], claim: string, kind: 'accepted' | 'orphan', startedAt: number, result: PurgeRunResult): Promise<void> {
    for (const row of items) {
      // Reserve enough wall time for bounded provider + callback. DB alone decides the µs due boundary.
      if (!this.#hasBudget(startedAt)) { result.deferred++; continue; }
      ...
      const context = record(await this.rpc(kind === 'accepted' ? 'get_photo_purge_context' : 'get_photo_orphan_purge_context', args));
      ...
      await this.rpc(kind === 'accepted' ? 'settle_photo_purge' : 'settle_photo_orphan_purge', { ...args, p_outcome: outcome, p_reason_code: reason });
      if (outcome === 'deleted') result.deleted++;
      else if (outcome === 'not_found') result.notFound++;
      else result.retryable++;
    }
  }
  async #folders(items: Record<string, unknown>[], claim: string, startedAt: number, result: PurgeRunResult): Promise<void> {
    for (const row of items) {
      if (!this.#hasBudget(startedAt)) { result.deferred++; continue; }
      const args = { p_folder_registry_id: uuid(row.folderRegistryId), p_lease_version: version(row.leaseVersion), p_claim_digest: claim };
      const context = record(await this.rpc('get_photo_folder_purge_context', args));
      ...
      try {
        const inspection = await this.provider.purgeEmptyFolder(folderId, parentId);
      ...
      await this.rpc('settle_photo_folder_purge', { ...args, p_outcome: outcome, p_reason_code: reason });
      if (outcome === 'deleted') result.deleted++; else if (outcome === 'not_found') result.notFound++; else result.retryable++;
    }
  }
  async run(): Promise<PurgeRunResult> {
    const startedAt = this.clock(), claim = await newPurgeClaim();
    const result: PurgeRunResult = { claimed: 0, deleted: 0, notFound: 0, retryable: 0, deferred: 0, acceptedClaimed: 0, orphanClaimed: 0, folderClaimed: 0 };
    try {
      const accepted = await this.#claim('claim_due_photo_purges', claim, PHOTO_PURGE_BATCH_LIMIT);
      result.acceptedClaimed = accepted.length; result.claimed += accepted.length;
      await this.#files(accepted, claim, 'accepted', startedAt, result);
      const orphan = await this.#claim('claim_due_photo_orphan_purges', claim, PHOTO_PURGE_BATCH_LIMIT - result.claimed);
      result.orphanClaimed = orphan.length; result.claimed += orphan.length;
      await this.#files(orphan, claim, 'orphan', startedAt, result);
      const folders = await this.#claim('claim_due_photo_folder_purges', claim, PHOTO_PURGE_BATCH_LIMIT - result.claimed);
      result.folderClaimed = folders.length; result.claimed += folders.length;
      await this.#folders(folders, claim, startedAt, result);
      await this.rpc('record_photo_purge_heartbeat', {
        p_status: result.retryable > 0 || result.deferred > 0 ? 'degraded' : 'succeeded', p_claimed: result.claimed,
        p_purged: result.deleted + result.notFound, p_retrying: result.retryable, p_blocked: 0,
      ...
      try { await this.rpc('record_photo_purge_heartbeat', { p_status: 'failed', p_claimed: result.claimed, p_purged: result.deleted + result.notFound,
        p_retrying: result.retryable, p_blocked: 0, p_accepted_claimed: result.acceptedClaimed, p_orphan_claimed: result.orphanClaimed,
        p_folder_claimed: result.folderClaimed, p_error_code: 'PHOTO_PURGE_FAILED' }); } catch { /* original failure remains authoritative */ }
      throw error;
    }
  }
}
...
export async function handlePhotoPurge(request: Request, secret: string | undefined, worker: () => Promise<PurgeRunResult>): Promise<Response> {
  ...
  try {
    ...
    return respond(200, { result: { claimed: result.claimed, deleted: result.deleted, notFound: result.notFound, retryable: result.retryable, deferred: result.deferred } });
