...
from .models import TRANSITIONAL_ACCOUNT_STATUSES, Account
...
def account_action_policy(account: Account, active_admin_count: int) -> AccountActionPolicy:
    ...
    transitional = account.status in TRANSITIONAL_ACCOUNT_STATUSES
    return AccountActionPolicy(
        can_change_role=not last_admin,
        can_change_status=not last_admin and not transitional,
        ...
        warning=(
            "마지막 active business admin은 역할·상태를 변경할 수 없습니다."
            if last_admin
            else "전이 상태 계정은 서버 lifecycle이 끝나기 전 상태를 직접 변경할 수 없습니다."
            if transitional
            else None
        ),
    )
