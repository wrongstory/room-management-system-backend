...
def main() -> None:
    ...
    if not isinstance(paths, dict) or len(paths) != 122:
        raise RuntimeError("전체 source OpenAPI path 수가 122가 아닙니다.")
    ...
    if operation_count != 133:
        raise RuntimeError("전체 source OpenAPI operation 수가 133이 아닙니다.")
