...
def main() -> None:
    ...
    if not isinstance(paths, dict) or len(paths) != 105:
        raise RuntimeError("전체 source OpenAPI path 수가 105가 아닙니다.")
    ...
    if operation_count != 112:
        raise RuntimeError("전체 source OpenAPI operation 수가 112가 아닙니다.")
