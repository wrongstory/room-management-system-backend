...
def main() -> None:
    ...
    if not isinstance(paths, dict) or len(paths) != 108:
        raise RuntimeError("전체 source OpenAPI path 수가 108이 아닙니다.")
    ...
    if operation_count != 115:
        raise RuntimeError("전체 source OpenAPI operation 수가 115가 아닙니다.")
